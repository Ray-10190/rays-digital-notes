...
打開你想要工作的資料夾（例如桌面或文件檔），在空白處按右鍵，選擇「在終端機中開啟」。
...
OpenCode 可以串接 **Memory** 伺服器，讓它跨對話記住你的資訊（偏好、決策紀錄等）。
...
OpenCode 會產出完整的計畫，包括頁面結構、功能清單、視覺設計建議。
